/*
 * file: PlayerStatsSummary.tsx
 * component: PlayerStatsSummary
 * description:
 *     Displays a summary of statistics for a specific player (wins, losses, draws).
 *     Fetches the data from the backend based on the provided playerName.
 */
...
const PlayerStatsSummary: React.FC<PlayerStatsSummaryProps> = ({ playerName }) => {
  ...
  useEffect(() => {
    ...
    // Only attempt fetching if a valid playerName is provided
    if (playerName) {
      loadPlayerStats();
    }
  }, [playerName]);
    ...
    <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-6 mb-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="text-center p-4 bg-yellow-50 dark:bg-yellow-900/20 rounded-lg">
          ...
          <div className="text-sm text-yellow-600 dark:text-yellow-400 mt-1">Wins</div>
        </div>
        ...
        <div className="text-center p-4 bg-red-50 dark:bg-red-900/20 rounded-lg">
          ...
          <div className="text-sm text-red-600 dark:text-red-400 mt-1">Losses</div>
        </div>
        ...
        <div className="text-center p-4 bg-blue-50 dark:bg-blue-900/20 rounded-lg">
          ...
          <div className="text-sm text-blue-600 dark:text-blue-400 mt-1">Draws</div>
        </div>
      </div>
    </div>
  );
